...
@inngest_client.create_function(
    ...
    # throttle و rate_limit قد لا تكون مدعومة في إصدارك الحالي
...
async def rag_ingest_pdf(ctx: inngest.Context) -> dict:  # type: ignore
    def _load(ctx: inngest.Context) -> RAGChunkAndSrc:  # type: ignore
        pdf_path = ctx.event.data["pdf_path"]  # type: ignore
        source_id = ctx.event.data.get("source_id", pdf_path)  # type: ignore
...
@inngest_client.create_function(
    fn_id="RAG: Query PDF",
    trigger=inngest.TriggerEvent(event="rag/query_pdf_ai"),  # type: ignore
)
async def rag_query_pdf_ai(ctx: inngest.Context) -> dict:  # type: ignore
    ...
    question = ctx.event.data["question"]  # type: ignore
    ...
    res = await ctx.step.ai.infer(  # type: ignore
        "llm-answer",
        adapter=adapter,
        body={
            "max_tokens": 1024,
